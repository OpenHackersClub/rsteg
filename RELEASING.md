...
- **Prebuilt binaries** — GitHub Releases with a curl-able shell installer, produced by [`cargo-dist`](https://opensource.axo.dev/cargo-dist/).
...
Pushing `vX.Y.Z` triggers `.github/workflows/release.yml` (generated by `cargo-dist`). It:

- builds `rsteg-cli` for 5 targets (`{x86_64,aarch64}-apple-darwin`, `{x86_64,aarch64}-unknown-linux-gnu`, `x86_64-pc-windows-msvc`)
- produces `.tar.xz` / `.zip` archives with `rsteg` + README + LICENSE
- generates shell (`rsteg-cli-installer.sh`) and PowerShell (`rsteg-cli-installer.ps1`) installers
- uploads everything to a new GitHub Release named `vX.Y.Z`
...
# GitHub Release shell installer
curl -LsSf https://github.com/OpenHackersClub/rsteg/releases/download/vX.Y.Z/rsteg-cli-installer.sh | sh
```
...
| Prebuilt binaries on tag push   | `release.yml` (generated by `cargo-dist`)    |
...
If you change `dist-workspace.toml` (adding a target, changing installers, bumping `cargo-dist-version`), regenerate the workflow:

```sh
cargo install cargo-dist --version 0.28.0 --locked   # match dist-workspace.toml
dist generate --mode=ci
git add .github/workflows/release.yml dist-workspace.toml
git commit -m "chore(release): regenerate cargo-dist workflow"
```
